...
- **`flight_server`** (`src/server/`) is a standalone binary with no Godot dependency. It accepts multiple concurrent human clients (`--max-players`, default 8 — increment 5's `--max-clients`, renamed once bots gave "how many players" and "how many aircraft" different answers), assigns each a `player_id`, steps every active aircraft across a worker-thread pool, applies each client's input through its own ordered command buffer fed by redundant `ControlInput` packets (robust to packet loss without needing the client to resend anything extra), and broadcasts chunked state snapshots at a configurable rate (30 Hz by default). `--stress-aircraft N` adds synthetic, unpiloted aircraft purely for load/chunking testing, drawing player IDs from a separate range so they never collide with real clients or bots. `--max-bots B` (default 0) forks up to `B` `flight_bot` processes to fill otherwise-empty airspace — see "Bots" below for the full capacity model. `--aircraft {c172x|camel|pa28}` (default `c172x`) picks the JSBSim aircraft config the whole server process (and every bot it spawns) flies for its lifetime, looked up from the small catalog in `src/aircraft_catalog.h`/`.cpp`; `--scenario` (below) requires `c172x` and is rejected at startup otherwise, since increment 1's scripted scenarios are themselves the c172x ground truth.
- **`flight_bot`** (`src/bot/`) is a genuinely headless client, spawned only by `flight_server` itself (never run standalone in normal use): the same real local `FlightSession` + `PredictedSession` a human's Godot client runs, just flying `bot::ManeuverController` — a closed-loop attitude-target autopilot (`src/bot/maneuver_controller.h`/`.cpp`) — instead of reading keyboard input. It self-declares as a bot in its own `ClientHello` (`kClientFlagIsBot`), which the server echoes back as a non-human `status_flags` bit on every snapshot entry for that aircraft, and exits cleanly on `SIGTERM` (a normal despawn) or the moment it loses its server connection (crash safety - a server crash never runs its own cleanup, so the bot exits itself rather than flying on orphaned).
- **`flight_test_client`** (`src/net_test/`) is the automated, Godot-free test harness used by `run_tests.sh` (see above); `--mode prediction` runs a real `PredictedSession` against a live server (`--aircraft` selects which catalog type it trims locally, matching the server's own), `--mode multiclient` connects several simulated clients in one process to exercise the server's multi-client machinery and `interpcore`'s live data path, `--mode mtu_regression` is a standalone serialization-size check with no server needed, `--mode aircraft_mismatch` exercises the aircraft-type mismatch handshake behaviour on its own (`--allow-aircraft-mismatch` is the override that lets a deliberately mismatched client stay connected instead of the default warn-and-disconnect), and `--mode observe_bots` connects as a plain observer and asserts that `--expect-bot-count` aircraft are marked non-human and genuinely evolving (not a frozen `--stress-aircraft`).
...
./scripts/build_server.sh                   # configures + builds only flight_server (skips godot-cpp entirely) - also builds flight_bot
./scripts/server.sh start --max-players 8   # starts it in the background; logs to run/server.log
...
Every additional Godot client that connects to the same server gets its own aircraft, and each client renders every other one automatically — `--max-players` (default 8) caps how many humans `flight_server` will accept on top of any bots (see "Bots" below):

```bash
./build/flight_server --max-players 8
```
...
```bash
./build/flight_server --max-players 4
./build/flight_test_client --mode multiclient --num-clients 3 --test-capacity
```

This asserts distinct player-ID assignment, that no client's aircraft responds to another client's input, live `interpcore`-based tracking of the other simulated clients' aircraft, and (with `--test-capacity`) that a connection beyond `--max-players` is correctly rejected and that a freed slot can be reused. `--stress-aircraft N` on the server (synthetic, unpiloted aircraft, never counted against `--max-players`) paired with `--expect-total-aircraft` on the client checks that a snapshot spanning more than one chunk (more than 16 aircraft) still reconstructs correctly with no duplicates.
...
`--max-bots B` fills otherwise-empty airspace with genuinely headless `flight_bot` processes — the same real local `FlightSession` + `PredictedSession` a human's Godot client runs, flying a closed-loop maneuver autopilot (a shallow-bank turn / gentle climb-descent / level cycle) instead of keyboard input:

```bash
./build/flight_server --max-bots 3 --max-players 8
```

The load model has two caps working together: the airspace fills to `B` bots when empty; up to `--max-players P` humans then join *on top* of the bots (airspace grows from `B` toward `B+P`); each human beyond `P` instead *displaces* one bot (airspace holds at `B+P`, shifting from bot to human), until at `B+P` humans it is all-human — true human capacity is `B+P`, not `P` alone, and a human is never turned away in favour of a bot; a disconnecting human refills a bot back toward `B`. This deliberately levels server CPU: a *local* bot costs the server host two JSBSim instances (its own prediction copy, forked, plus the server's authoritative copy) where a human costs one (their prediction runs on their own machine), so load peaks at the bots-plus-players transition and *eases* as further humans displace bots — provision for `2B + P` instances and it never overloads however the mix shifts.

Every bot self-declares in its own `ClientHello` (the same interface a future third-party or remote bot would use — see `docs/roadmap.md`, "Bot platform and third-party bots"), which the server echoes back as a non-human marker on that aircraft's `status_flags`; the Godot client renders bot aircraft with a distinct marker from human players. Despawning a bot is a plain `SIGTERM` (`flight_bot` treats it as a clean disconnect); it also exits the moment it loses its own server connection, so a crashed `flight_server` (which never gets to run its own cleanup) still leaves no orphaned bot processes behind.
...
The script prints one line per test with its `passed`, `failed`, or `error` status; any failed criterion is listed underneath with its name, measured value, and limit. The script's own exit status is `0` only when every test — increment 1's four, increment 2's five, increment 3's networked phase, increment 4's prediction phase, increment 5's multi-core/multi-client phase, increment 6's multi-airframe phase, and increment 7's bot phase — passed.
...
`flight_test_client --mode multiclient`/`mtu_regression`/`aircraft_mismatch`/`observe_bots` print their own pass/fail lines directly (distinct player IDs, no cross-talk, chunked-aircraft-count, live remote tracking, capacity/slot-reuse, MTU threshold, aircraft-type mismatch/override/matched behaviour, bot marking/liveness) rather than writing a CSV log — there is no single "the" client whose predicted trajectory to log, since the point of those modes is exercising the wire-level/multi-client behaviour itself, not one aircraft's flight path. `catalog_tests`/`bot_endurance_gate_test` (the standalone regression tests) likewise only print a pass/fail line per catalog entry, with no CSV output.
...
- [`docs/increment-6-specification.md`](docs/increment-6-specification.md) / [`docs/increment-6-specification-review.md`](docs/increment-6-specification-review.md)
- [`docs/increment-7-specification.md`](docs/increment-7-specification.md) / [`docs/increment-7-specification-review.md`](docs/increment-7-specification-review.md) — this increment's specification and adversarial review, including the empirical findings that shaped it (trim-blindness and open-loop control both depart every airframe, the local-physics decision this motivated, and the Camel-specific maneuver tuning that let it join the bot fleet after all)
