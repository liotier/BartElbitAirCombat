...
# Keyboard -> FlightAircraft control mapping, per
# docs/increment-2-specification.md ("Input handling"). Placed before
# FlightAircraft in the scene tree so this runs first each physics tick.
extends Node

const THROTTLE_RATE := 1.0 / 3.0  # full range in ~3s, per spec

var _aircraft: FlightAircraft


func _ready() -> void:
	_aircraft = get_node("../FlightAircraft")
	_aircraft.initialize()
	_aircraft.set_initial_condition(5000.0, 100.0, 0.0, 0.0, 0.0, 0.0)
	_aircraft.trim()
