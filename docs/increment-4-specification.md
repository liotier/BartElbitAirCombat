...
- JSBSim exposes the building blocks (`FGPropagate::GetTl2i()`, `GetEarthPositionAngle()`, `SetInertialOrientation()`) needed to reconstruct the ECI-frame `VehicleState` `SetVState()` expects from wire-format local-frame fields — but the reconstruction itself is unverified and is a **required pre-implementation gate** (see "Reconstruction gate").
...
6. If beyond threshold — **reconciliation**:
   a. Reconstruct a JSBSim `VehicleState` from the snapshot's authoritative fields — **exactly as validated by the reconstruction gate below**, not improvised here: position (local offset inverted against the known origin from `ServerWelcome`, giving lat/lon; altitude directly), attitude (`ang_vel_body_rps` gives `vPQR` directly, no rotation; the wire's local quaternion converted to the ECI quaternion `SetVState()` requires using the client's own `GetTl2i()`/`GetEarthPositionAngle()` at the target position — both computable independently of anything that diverged, since Earth Position Angle depends only on elapsed simulation time), velocity (rotate the wire's local-frame velocity into body frame using the now-known attitude).
...
## Reconstruction gate (required, before implementation proceeds)

The reconciliation `VehicleState` reconstruction (step 6a) is the one place reconciliation can *corrupt* state rather than merely fail to help — a wrong local→ECI attitude composition `SetVState()`s the aircraft to a wrong orientation, worse than not reconciling. Draft 1 left this as "the first implementation task"; review finding M2 elevates it to a **hard gate with an explicit pass criterion**, because the building blocks were confirmed to exist and be public (`GetTl2i`, `GetEarthPositionAngle`, `SetInertialOrientation`), so it is verifiable up front:

> Take a trimmed instance flown to a non-trivial attitude. Read its wire-level fields (local-frame position, `geo::computeOrientationQuat()` local quaternion, local-frame velocity, body-frame `p`,`q`,`r`). Reconstruct a `VehicleState` from *only* those fields and `SetVState()` a **fresh** trimmed instance to it. Read back Euler angles, position, velocity, and body rates and require agreement with the source within tolerance (the float32 transport floor, ~10⁻³°, is the natural bound).

Implementation does not proceed past a failing gate. If the frame algebra proves subtle enough to need real coordinate-frame judgment, that is the point to escalate (review "Implementation model recommendation").
...
1. The reconstruction gate has been passed (a wire-field → `SetVState()` → read-back round trip agrees within tolerance) — this precedes and unblocks the rest of implementation.
...
JSBSim API surface this increment relies on beyond increment 3's usage: `FGPropagate::GetVState()`/`SetVState()` (now the confirmed rigid-body mechanism), `FGPropagate::GetTl2i()`, `FGPropagate::GetEarthPositionAngle()`, `SetInertialOrientation()`, and the property-tree reads `velocities/p-rad_sec`, `velocities/q-rad_sec`, `velocities/r-rad_sec`.
...
- **The ECI-attitude-reconstruction building blocks exist and are public**: `FGPropagate::GetTl2i()`, `GetEarthPositionAngle()`, and `SetInertialOrientation()` are all public, confirming the reconstruction step is implementable against JSBSim's real API rather than requiring a JSBSim patch. The exact composition was identified (`Tl2i.GetQuaternion() * qLocal`) but not yet round-trip-tested — that round trip is the **reconstruction gate** (see body), promoted from "first task" to a hard pre-implementation gate by review finding M2.
