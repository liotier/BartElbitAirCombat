...
Download the official precompiled Godot 4.5.x stable Linux editor build directly (a plain ~80–100 MB HTTPS download — e.g. from `https://github.com/godotengine/godot/releases` or `https://godotengine.org/download/archive/`). **Do not build Godot from source.** Building from source is what this specification's own validation work did, and it is not representative of what the project needs: it was a workaround specific to the sandboxed environment used for that validation, which could fetch git repositories but could not download release binaries directly. A normal developer machine or CI runner has no such restriction; a source build takes on the order of 30 minutes on 4 cores (measured — see Appendix B) against roughly 30 seconds for the official binary.

This specific step — the binary download — was **not** empirically exercised while drafting this specification, for the reason above. Verify the exact release asset URL and filename at implementation time, in the same spirit as increment 1's JSBSim tag-verification caution.
