...
Revision 5. Pre-drafting validation (process/lifecycle) plus adversarial review (`docs/increment-7-specification-review.md`) folded in, then the review's one open architecture decision resolved by the project owner: **bots run their own local JSBSim** (a true headless client, not a physics-less one — see "The local-physics decision" below), along with the owner's capacity/load model and non-human marking (revision 3). Revision 4 adds two owner-directed refinements from the bot-interface discussion: **one unified bot interface** — local *and* remote bots self-declare identically via `ClientHello`, so there is a single server-facing bot interface and a structurally guaranteed level playing field — and a **clean bot core / intelligence seam**, the internal boundary that becomes the eventual public third-party bot SDK (self-hosted by default — no sandbox needed, since the wire protocol already isolates any connecting intelligence exactly as it does a human's client; WASM is a separate, narrower option for an operator who chooses to run bot code inside their own server process — a named future increment, `docs/roadmap.md` "Bot platform and third-party bots"). Revision 5 is a cross-spec coherence pass before implementation: it pins the bot's aircraft-type acquisition to increment 6's trim-before-connect sequence (type passed via argv at fork, not adopted from `ServerWelcome` — that alternative stays deferred in increment 6), and states the capacity model's terminal case (`kServerFull` only at B + P humans with no bot left) and flag defaults. Not yet implemented.
...
New standalone binary, `src/bot/main.cpp`, Godot-free, depending on `flightcore` (its own `FlightSession`), `predictcore` (`PredictedSession` — predict/reconcile, reused unmodified from the Godot client), `netcore`, and `interpcore`. On startup it loads and trims its aircraft type **before connecting** — the same trim-before-connect sequence increment 6 specifies for `PredictedAircraft` (whose adopt-the-type-from-`ServerWelcome` alternative remains explicitly deferred there, and is not quietly resurrected here). A *local* bot gets its type the same way it gets the server's port: passed by the server at `fork()`/`exec()` time via argv, since the server knows its own `--aircraft` — no wire round-trip needed. The welcome's `aircraft_id` then serves as increment 6's standard mismatch check, which for a server-spawned bot can only fire on a genuine bug — in which case the bot warns and exits (increment 6's default mismatch behaviour) rather than flying mispredicted. After that, it runs the identical predict/send/reconcile loop — the bot *is* increment 4/5's client, headless.
...
The seam is deliberate and load-bearing for the project's direction, not incidental structure: it is the exact boundary that becomes the **public third-party bot SDK** — the core is the platform we publish, the intelligence is what a third party (or we) writes against it. The wire protocol and server authority already isolate any connecting intelligence uniformly (a bot's decision logic is no more or less trusted by the server than a human's own reasoning is), so an author who runs their own intelligence on their own machine — native code, or an external process/LLM calling the same narrow API over a protocol like MCP — needs no sandbox at all; the future increment that publishes the SDK can also add a **WASM** host, but that answers a different, narrower question (can a *server operator* safely run bot code, official or trusted-third-party, inside their *own* server process) than "can a bot compete safely," which the interface already guarantees. Drawing the seam cleanly now, while it is all C++ and all ours, means either path can be added later without touching the core. Two rules make the seam real from day one: the core carries no `if (this particular maneuver)` logic, and the intelligence never reaches past the API to the ENet/JSBSim internals directly. Nothing WASM- or MCP-related is *built* here; only the seam that makes both cheap later. See "Out of scope" for the deferred platform increment.
...
Policy (the owner's load model): the airspace fills to **B** bots when empty. As humans connect, up to **P** of them are admitted on top of the bots — the airspace grows from B toward **B + P** — and each human beyond P is still admitted, now by **displacing one bot** (the airspace holds at B + P, its composition shifting from bot to human), until at B + P humans it is all-human with no bots. Total aircraft never exceeds **B + P**, and a human is never turned away in favour of a bot. **The terminal case is stated, not implied:** a human is rejected (`kServerFull`, increment 5's existing rejection) only when the airspace already holds B + P aircraft *and* no bot remains to displace — i.e. the true human capacity is **B + P**, and `--max-players P` is deliberately the *additive threshold* (how many humans join before bots start yielding), not a hard player cap; the implementer must not "correct" it into one. Defaults: `--max-bots 0`, `--max-players 8` — with zero bots, `--max-players` behaves exactly like increment 5's `--max-clients` (which it replaces).
...
**Remote / third-party bot hosting itself is still deferred** (the "bot-programmer competition scene"): what this increment builds is *local* bots, but on the unified interface, so a future increment adds remote bots by (a) publishing the bot core + intelligence-seam as an SDK so authors can self-host a native or MCP-driven intelligence with no sandbox needed (the wire protocol already isolates it, exactly as it does a human's client), (b) separately, a WASM host for the case where a server *operator* wants to run bot code — official or trusted-third-party — inside their own server process, (c) teaching the CPU-leveling model that a self-hosted remote bot costs one server-host instance, not two, and (d) defining **displacement for remote bots** — this increment's despawn mechanism is `SIGTERM`, which only exists for server-forked local processes; whether a remote bot yields its slot to a human via a wire-level leave request, or is simply exempt from displacement (occupying a slot on human terms, since its author runs it deliberately), is a policy decision for that increment, flagged here so it is made and not stumbled into — with **no change to the server-facing interface**, which is already what a remote bot would use.
...
3. `flight_server --max-bots B --max-players P` implements the capacity/CPU-leveling model: bots fill to B when empty; humans add on top up to P then displace bots; total never exceeds B + P; a human is never rejected for a bot, and is rejected (`kServerFull`) only at B + P humans with no bot left to displace.
...
Everything in increments 1-6's deferred lists, plus: combat intelligence / energy-management AI and the per-bot handicap factor (a later increment — local physics is built now *for* it, but no smarts here); the **bot platform** — publishing the bot core + intelligence seam as a public API, an MCP server for self-hosted external/LLM intelligences, a WASM host for operators who want to run bot code (official or trusted-third-party) inside their own server process, the "bot-programmer competition" scene, and any scoring framework (its own named future increment, `docs/roadmap.md` "Bot platform and third-party bots"); difficulty/personality tuning; persistent bot identity across restarts; multiple aircraft types per bot roster (increment 8). Note: this increment *does* build the unified server-facing bot interface (the `ClientHello` self-declaration) and the internal core/intelligence seam, precisely so that future increment adds self-hosted intelligences and the operator-run WASM path without reopening either.
