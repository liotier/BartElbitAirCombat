...
- **Elevator/aileron/rudder are `int16`, fixed-point** (value × 32767, clamped to [-1,1]). A stick cannot be commanded finer than 1/32767; `int16` is a comfortable-margin narrow default. These three axes set rotation *rates*, where visible quantization would read as jitter in how the aircraft turns — matching how Quake-lineage netcode (`usercmd_t`) gives view/aim angles 16 bits while movement axes get only 8.
- **Throttle is `uint8`, fixed-point** (value × 255, clamped to [0,1]) — unsigned rather than reusing the signed axis encoding, since throttle never goes negative and a signed type would waste half its range for no reason (an int16 encoding was tried first and found to do exactly that). Throttle is also a coarse, movement-like axis in the same sense `usercmd_t`'s `forwardmove`/`rightmove`/`upmove` are (a `signed char`, i.e. one byte, in that struct) — tolerant of coarser quantization than a rotation-rate axis.
...
| 4 | `ControlInput` | C→S | 1 | unreliable | `uint32` client_seq, `int16` elevator, `int16` aileron, `int16` rudder, `uint8` throttle |
...
| throttle | `uint8` | round(value × 255), value ∈ [0, 1] |
...
- **Throttle narrowed from `int16` to `uint8`** (post-implementation revision, resolving review finding m3): the increment shipped with throttle at the same `int16` encoding as the other three axes, using only 0…32767 of its range - exactly the waste m3 flagged as optional to fix. Revisited once real-world precedent was checked directly rather than assumed: Quake 3's own `usercmd_t` (`forwardmove`/`rightmove`/`upmove`) uses a `signed char` - one byte - for its coarse movement axes, reserving 16-bit precision for view/aim angles specifically. Throttle is the movement-like axis of this protocol's four (elevator/aileron/rudder set rotation *rates*, where coarser quantization would be more perceptible as jitter); `uint8` removes the wasted range entirely (throttle is never negative) rather than merely halving it, and better matches that precedent than either leaving it at `int16` or widening to `uint16` would have. Changed in `protocol.h`/`.cpp` (`encodeThrottle`/`decodeThrottle`, a dedicated `kThrottleScale = 255.0`) and its three call sites; full suite re-verified green after the change.
